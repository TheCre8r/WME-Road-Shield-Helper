...
(function() {
    ...
    const SCRIPT_CHANGES = `Initial Version`
    ...
    function RegexMatch() {
        ...
        document.querySelector("#WMERSH-Autofill").onclick = function(){
            let streetname = document.querySelector("#wz-dialog-container > div > wz-dialog > wz-dialog-header > div.street-name").innerText
            console.log(streetname)
            let regex = /(?:(H|I|M|(?:[A-Z]\w)(?=\-))-(\d+(?:[A-Z])?(?:-\d+)?))?(?: (BUS|ALT|BYP|CONN|SPUR|TRUCK))?(?: (N|E|S|W))?/;
            ...
            switch (match[1] ) {
                ...
                case "SR":
                    if (DoneStates.indexOf(State) == -1 ) {
                        CreateAlert(`Warning: State Shield Not Verified.<br>Consult local guidance and <a target="_blank" href="https://github.com/TheCre8r/WME-Road-Shields-Filter/issues/new" id="WMERSH-report-an-issue">${I18n.t(`wmersh.report_an_issue`)}</a>`)
                    }
    ...
    function initTab() {
        ...
        $section.html([
            ...
            `<a target="_blank" href="https://github.com/TheCre8r/WME-Road-Shields-Filter/issues/new" id="WMERSH-report-an-issue">${I18n.t(`wmersh.report_an_issue`)}</a>`,
            '</div>',
            '</div>',
            ...
            `<div style="display: inline-block;">`,
            `<a target="_blank" href="https://github.com/TheCre8r/WME-Road-Shields-Filter/wiki" id="WMERSH-help-link">${I18n.t(`wmersh.help`)}</a>`,
            `</div>`,
            `</div>`,
            '</div>',
            '</form>',
